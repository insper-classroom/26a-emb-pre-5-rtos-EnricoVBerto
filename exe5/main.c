...
SemaphoreHandle_t yellowLedSemaphore;
...
void btn_callback(uint gpio, uint32_t events) {
    ...
    if (btnEvent != 0) {
        xQueueSendFromISR(buttonQueue, &btnEvent, NULL);
    }
}
...
void redLedTask(void *p) {
    ...
    while (true) {
        if (xSemaphoreTake(redLedSemaphore, 0) == pdTRUE) {
...
void yellowLedTask(void *p) {      
    ...
    while (true) {
        if (xSemaphoreTake(yellowLedSemaphore, 0) == pdTRUE) {
...
void buttonTask(void *p) {
    ...
    while (true) {
        if (xQueueReceive(buttonQueue, &receivedBtnEvent, portMAX_DELAY) == pdTRUE) {
            if (receivedBtnEvent == BTN_RED_EVENT) {
                xSemaphoreGive(redLedSemaphore);
            } else if (receivedBtnEvent == BTN_YELLOW_EVENT) {
                xSemaphoreGive(yellowLedSemaphore);
            }
        }
    }
}
...
int main() {
    ...
    redLedSemaphore = xSemaphoreCreateBinary();
    yellowLedSemaphore = xSemaphoreCreateBinary();
